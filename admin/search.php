...
<?php }elseif($act == "query") {
    ...
    if($type == 1){
        $sql = "select * from ayangw_km where km like '%{$txt}%'";
    }else if($type == 2){
        $sql = "select * from ayangw_km where out_trade_no like '%{$txt}%' or trade_no like '%{$txt}%'";
    }else if($type == 3){
        $sql = "select * from ayangw_km where rel like '%{$txt}%'";
    }
   // echo $sql;
    $rsss=$DB->query($sql);
   // echo $rsss;
